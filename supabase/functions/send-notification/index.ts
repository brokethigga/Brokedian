...
async function readPayload(req: Request): Promise<NotificationPayload | null> {
  try {
    const payload = await req.json()
    const type = sanitizeText(payload?.type, 64)
...
serve(async (req) => {
  try {
    ...
    if (!RESEND_API_KEY) {
      ...
      return jsonResponse({ ok: true, note: 'email_skipped' })
    }
        ...
        <div style="font-family: 'DM Sans', sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px; background: #f5f5f7;">
          <div style="background: #fff; border-radius: 16px; padding: 32px 24px; box-shadow: 0 2px 12px rgba(0,0,0,0.06);">
            <div style="text-align: center; margin-bottom: 24px;">
              ...
              <h2 style="font-family: 'Syne', sans-serif; font-size: 20px; font-weight: 700; color: #1D1D1F; margin: 0 0 4px;">${safeTitle}</h2>
              <p style="font-size: 15px; color: #6E6E73; margin: 0;">${safeBody}</p>
            </div>
    ...
    return jsonResponse({ error: 'notification_failed' }, 500)
  }
})
